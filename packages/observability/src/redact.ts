 ...
 *    `REDACTED_HOME_PATH_USER` is the placeholder we substitute. The
 *    substitution is *global* (anywhere in the string), not just at the
 *    start, so `"Executing command in /home/sande/project"` becomes
 *    `"Executing command in ~/project"`.
 *
...
export function redactHomePath(input: string): string {
  ...
  // Escape the home path for a RegExp literal, then replace every
  // occurrence that begins a path segment (start of string, or after a
  // non-word char) and ends at a path separator / end of string.
  const escaped = home.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
  return input.replace(
    new RegExp(`(^|[^\\w])${escaped}(?=$|[\\\\/])`, "g"),
    (_match, prefix: string) => `${prefix}${REDACTED_HOME_PATH_USER}`,
  );
}
