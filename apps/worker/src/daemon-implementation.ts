...
export class WorkerDaemon {
  ...
  private agentSource: FileAgentConfigSource;
  ...
  private legacySessionExecutor?: (request: Record<string, unknown>) => Promise<{
    sessionId?: string;
    status?: string;
    summary?: string;
  }>;
  private durableAttemptExecutorOverride?: (
    input: Parameters<WorkerDaemon["executeDurableAttempt"]>[0],
  ...
  /** Narrow seams used by worker behavior tests; production never calls them. */
  public setAgentLookupForTests(source: Pick<FileAgentConfigSource, "has" | "get">): void {
    this.agentSource = source as FileAgentConfigSource;
  }

  public setDurableAttemptExecutorForTests(executor: unknown): void {
    this.durableAttemptExecutorOverride = executor as WorkerDaemon["executeDurableAttempt"];
  }

  public setLegacySessionExecutorForTests(executor: unknown): void {
    this.legacySessionExecutor = executor as WorkerDaemon["legacySessionExecutor"];
  }
   ...
   */
  public async pollWakeups(): Promise<void> {
    if (this.shuttingDown) return;
  ...
  public async claimAndRun(wakeupId: string): Promise<void> {
    const handle = getDefaultDb();
  ...
  public async applyDiscoveryProposal(
  ...
  public async releaseRetainedManagerWorkspace(
    workspaceId: string,
  ...
  public async recoverDelegationCallbacks(): Promise<void> {
  ...
  public async executeDurableAttempt(input: {
    attempt: AgentAttempt;
    workItem: ExecutionWorkItem;
    agentId: string;
    adapter: string;
    ...
    if (this.durableAttemptExecutorOverride) return this.durableAttemptExecutorOverride(input);
...
`,
  ...
  public async queueDelegatedWork(
    action: HireAndDelegateAction,
    workItem: ExecutionWorkItem,
    adapter: string,
    manager: {
  ...
  public async persistAttemptOutput(input: {
    result: AdapterExecutionResult;
    attempt: AgentAttempt;
    workItem: ExecutionWorkItem;
  ...
  public async restorePreviousAttemptArtifacts(
    attempt: AgentAttempt,
    workspacePath: string,
  ): Promise<void> {
  ...
  public async recoverStaleClaims(): Promise<void> {
  ...
  public async recoverMissedExecutableWakeups(): Promise<void> {
    const handle = getDefaultDb();
