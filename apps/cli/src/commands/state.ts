...
export function stateCommand(): Command {
  ...
  cmd
    .command("show")
    .description("Show the current STATE.md (last runs, high priority items)")
    .action(async () => {
      ...
      try {
        ...
        console.log(pc.cyan("# Loop State"));
        console.log("");
        ...
        if (recentWakeups.length === 0) {
          ...
          for (const w of recentWakeups) {
            console.log(`  - ${w.id.slice(0, 20)}...  ${pc.gray(w.status)}  ${w.reason ?? ""}`);
          }
        }
        console.log("");
