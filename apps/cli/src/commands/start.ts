...
import { PatternRegistry, KillSwitch, Scheduler, STARTER_PATTERNS } from "@aaspai/loops";
...
export function startCommand(): Command {
  ...
  cmd
    .option("--once", "run a single tick and exit (for cron)", false)
    .action(async (opts) => {
      ...
      console.log(pc.gray(`  registered ${fileLoopIds.length} file-based loops + ${STARTER_PATTERNS.length} starter patterns`));
